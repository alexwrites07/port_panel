...
const AdminPanel = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const mediaSnapshot = await database.get('students');
    ...
    const handleDelete = async (uid) => {
      try {
        await database.remove(`students/${uid}`);
    ...
    const handleFormSubmit = async (e) => {
      ...
      try {
        ...
        if (formData.heading.length < 5 || formData.description.length < 5) {
          alert('Heading or description should be at least 75 characters.');
          return; // Prevent form submission
        }
    
        if (editingUid) {
          await database.set(`students/${editingUid}`, {
            img: imgURL,
